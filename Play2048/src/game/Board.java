...
public class Board {
    ...
    public void addRandomTile() {
        // generate a number from 0 to size of openSpaces
        int a = openSpaces.size();
        int r = StdRandom.uniform(a);
        // get the row and column of the boardspot object that was chosen
        int col = openSpaces.get(r).getCol();
        int row = openSpaces.get(r).getRow();
        // generate a randome number from 0 to 1 to decide if the new tile will be a 4
        // or a 2
        double d = StdRandom.uniform(0.0, 1.0);
        if (d < .1) {
            gameBoard[row][col] = 4;
        } else {
            gameBoard[row][col] = 2;
     ...
     * (ex: 4 4 0 0 becomes 4 4 0 0).
     **/
    public void swipeLeft() {
        for (int i = 0; i < gameBoard.length; i++) {
            for (int j = 0; j < gameBoard[0].length; j++) {
                if (gameBoard[i][j] == 0) {
                    for (int m = j + 1; m < gameBoard[0].length; m++) {
                        if (gameBoard[i][m] > 0) {
                            int temp = gameBoard[i][m];
                            gameBoard[i][m] = gameBoard[i][j];
                            gameBoard[i][j] = temp;
                            break;
                        }
    ...
    public void flipRows() {
        ...
        for (int i = 0; i < gameBoard.length; i++) {
            for (int j = 0; j < gameBoard[0].length; j++) {
                ...
                tempBoard[i][j] = gameBoard[i][gameBoard[0].length - 1 - j];
            }
        }
        for (int i = 0; i < gameBoard.length; i++) {
            for (int j = 0; j < gameBoard[0].length; j++) {
