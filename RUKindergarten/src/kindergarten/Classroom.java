...
public class Classroom {
    ...
    public void enterClassroom ( String filename ) {
        ...
        for(int i=0;i<rows;i++){
            if(studentsInLine==null){
                ...
                Student x= new Student(first, last, height);
                SNode n=new SNode();
               n.setStudent(x);
               studentsInLine=n;
            }
                ...
                String last=StdIn.readString();
                int height=StdIn.readInt();
                Student y= new Student(first, last, height);
                SNode f= new SNode(y,studentsInLine);
                studentsInLine=f;
            }
        }    
    }
    ...
    public void seatStudents () {
        ...
        Student s=new Student();
            for(int j=0;j<seatingLocation.length;j++){
               if(i==false){
                break;
               }
                for(int g=0; g<seatingLocation[0].length;g++){
                    if(seatingLocation[j][g]==true && studentsSitting[j][g]==null){
                        s=this.studentsInLine.getStudent();
                        studentsSitting[j][g]=s;
                        SNode prev=studentsInLine;
                        studentsInLine=studentsInLine.getNext();
                        prev=null;
                        if(this.studentsInLine==null){
                            i=false;
                            break;
                        }
                    }
    ...
    public void insertMusicalChairs (){
        for(int j=0;j<studentsSitting.length;j++){
             for(int g=0; g<studentsSitting[0].length;g++){
                if(studentsSitting[j][g]!=null){
                    if(musicalChairs==null){
                        SNode n=new SNode();
                        n.setStudent(studentsSitting[j][g]);
                        n.setNext(n);
                        musicalChairs=n;
                        studentsSitting[j][g]=null;
                    }
                    else{
                        SNode f=new SNode();
                        f.setStudent(studentsSitting[j][g]);
                        f.setNext(musicalChairs.getNext());
                        musicalChairs.setNext(f);
                        musicalChairs=f;
                        studentsSitting[j][g]=null;
                    }
                }
    ...
    public void moveStudentFromChairsToLine(int size) {
        ...
        SNode pnt=musicalChairs.getNext();
        SNode prev= musicalChairs;
        for(int i=0;i<s;i++){
            pnt=pnt.getNext();
            prev=prev.getNext();
        } 
        if(pnt==musicalChairs){
            musicalChairs=prev;
            musicalChairs.setNext(pnt.getNext());
        }else{
            prev.setNext(pnt.getNext());
        }
        Student r=pnt.getStudent();
        insertByHeight(r);
    }
    ...
    public void insertByHeight(Student studentToInsert) {
        int h=studentToInsert.getHeight();
        if(studentsInLine==null){
            SNode n=new SNode(studentToInsert,null);  
            studentsInLine=n;
        }else if(studentsInLine.getNext()==null){
            SNode pt=new SNode(studentToInsert, null);
            int c=studentsInLine.getStudent().getHeight();
            if(h>c || h==c){
                studentsInLine.setNext(pt);
            }
            if(h<c ){
                pt.setNext(studentsInLine);
                studentsInLine=pt;
            }
        }else{
            SNode prev=studentsInLine;
            SNode ptr=studentsInLine.getNext();
            SNode neww=new SNode(studentToInsert,null);
            while(ptr!=null){
                int ptH=ptr.getStudent().getHeight();
                int prH=prev.getStudent().getHeight();

                if((h>prH  && h<ptH) || (h==prH && h<ptH) ){
                    prev.setNext(neww);
                    neww.setNext(ptr);
                    break;
                }
                if(h>prH || (h==prH && h==ptH)) {
                    if(ptr.getNext()==null){
                        ptr.setNext(neww);
                        break;
                    }else{
